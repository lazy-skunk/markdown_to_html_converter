...
def _create_test_file(directory, filename, content):
    with open(os.path.join(directory, filename), "w", encoding="utf-8") as f:
        f.write(content)


def _cleanup_test_directory(directory):
...
def test_get_md_files(setup_test_environment, mock_logger):
...
def test_get_md_files_oserror(mock_logger):
    converter = MarkdownToHtmlConverter(mock_logger)
...
def test_read_md_file_error(mock_logger):
    converter = MarkdownToHtmlConverter(mock_logger)
    converter._CURRENT_DIRECTORY = "non_existing_dir"
    content = converter._read_md_file("non_existing_file.md")
    assert content == "", "Content should be an empty string for non-existing file"
    mock_logger.error.assert_called_with("File not found: non_existing_file.md")


def test_read_md_file_ioerror(mock_logger):
    converter = MarkdownToHtmlConverter(mock_logger)
    converter._CURRENT_DIRECTORY = "test_dir"
...
def test_convert_md_to_html_error(mock_logger):
    converter = MarkdownToHtmlConverter(mock_logger)
    with patch("markdown.markdown", side_effect=Exception("Conversion error")):
        html_content = converter._convert_md_content_to_html_content("invalid markdown")
        assert html_content == ""
        mock_logger.error.assert_called_with(
...
def test_write_html_file_error(mock_logger):
    converter = MarkdownToHtmlConverter(mock_logger)
    with patch("builtins.open", mock_open()) as mocked_open:
...
def test_main(setup_test_environment, mock_logger):
    test_dir, _, _, html_filename = setup_test_environment
    converter = MarkdownToHtmlConverter(mock_logger)
    converter._CURRENT_DIRECTORY = test_dir
    converter.main()
...
def test_main_no_md_files(setup_empty_directory, mock_logger):
...
def test_main_convert_md_content_to_html_content_empty(
    mock_logger, setup_test_environment
):
    test_dir, md_filename, _, _ = setup_test_environment
    converter = MarkdownToHtmlConverter(mock_logger)
    converter._CURRENT_DIRECTORY = test_dir
    ...
    with patch.object(converter, "_get_md_files", return_value=[md_filename]):
        with patch.object(
            ...
            converter.main()
            mock_logger.info.assert_any_call("Starting Markdown to HTML conversion.")
